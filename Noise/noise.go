...
func main() {
	...
	frequency := float32(0.001)
	gain := float32(0.2)
	lac := float32(3.0)

	// frequency = float32(clampFloat(0.0001, 0.01, frequency))
	// lac = float32(clampFloat(1.0, 5.0, lac))
...
/* for y := 0; y < winHeight; y++ {
		for x := 0; x < winWidth; x++ {
			noise[i] = (turbulence(float32(x)/float32(winWidth), float32(y/winHeight), frequency, lac, gain, octaves))
	...
	gradient := getDualGradientCOlor(color{0, 0, 17}, color{80, 160, 244}, color{12, 192, 75}, color{255, 255, 255})
...
// func colorGradient(c1, c2 color) []color {
